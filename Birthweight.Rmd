...
We opt for birthweight instead as the distribution is more well-behaved. 
...
We additionally take average cigarette smoked `cigs`.
...
We also add mother's age and squared age `mage` and `magesq` for dimishing return, average drink, and whether baby is male.
...
multiplot(cols=3,
  qplot(x=npvis, y=bwght, data=bwdt, geom='jitter', alpha=I(1/20)),
  qplot(x=npvissq, y=bwght, data=bwdt, geom='jitter', alpha=I(1/20)),
  qplot(x=monpre, y=bwght, data=bwdt, geom='jitter', alpha=I(1/20))
)
```
...
```{r}
ggplot(melt(bwdt[,c('bwght','npvis','npvissq','monpre')]), aes(x=value)) +
    geom_histogram(bins=30) +
    facet_wrap(~variable, scales='free') +
    ggtitle('Histograms of non-indicator variables')
```
...
```{r}
bwdt[,apgar2:=exp(fmaps)+bwght]
modnl1<-lm(apgar2 ~ npvis + npvissq + monpre + cigs + mage + magesq + male + drink, data=bwdt)
summary(modnl1)
par.orig <- par(no.readonly=T); par(mfrow=c(2,2)) # plot diagnostics on 2x2
plot(modnl1)
par(par.orig)
