...
As for `mage`, it's not intuitive that the mother's age would correlate positively or negatively with either `npvis` or `bwght`. Evaluating the restricted model shows little change without `mage` shows little change in the coeffcient for `npvis`.


```{r}
lm(bwght ~ npvis + npvissq + monpre + cigs, data=bwdt)
lm(bwght ~ npvis + npvissq + monpre, data=bwdt)
lm(bwght ~ npvis + npvissq + monpre + mage + magesq, data=bwdt)
```
